...
| the context nudge | the same rule without having to remember it. A `UserPromptSubmit` hook that says, once per band and never again until the band changes, that it is time to compact. HOLD is silent, which is most prompts. Register `hooks/context-nudge.sh`, never the versioned Python path. |
...
| the context nudge | 35 cases in CI, most of them proving silence, and 13 mutations. Writing them found two real defects: the state never came back down, so a session that compacted and refilled was never warned again, and the blanket `except Exception` made a crash indistinguishable from correct silence |
